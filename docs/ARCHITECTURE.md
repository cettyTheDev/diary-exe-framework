...
- The fixture Source Files view exercises the dossier, document-density map, page-frame navigator, paper review surface, and selected-record inspector without a real source identity, source URL, checksum, quotation, or page image. Every interactive frame remains explicitly marked as demo data.
- `lib/archive/source-links.ts` and the shared `SourcePageLink` keep fixture navigation page-exact across Receipts, Timeline, Board evidence cards, Trace, and Source Files. They demonstrate the handoff contract without bundling a production source or official-source action.
