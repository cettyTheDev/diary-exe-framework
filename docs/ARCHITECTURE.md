...
- `lib/archive/board-layout.ts` keeps small graphs on a deterministic degree-centered orbit and moves dense 16–24-node graphs to a separated perimeter. On wide screens the Board renders the highest-degree node as a circular dossier hub, varies peripheral markers by entity kind, and places the graph over orbit/dot guides with bounded zoom/pan, keyboard controls, and a direct-connection lens. Below 72rem it falls back to a full-width stacked reading list without trapping touch scrolling.
