...
- `ArchiveShell` also owns fixture-labeled same-page quick-entry actions and a four-step demo evidence path. Stable view anchors exercise the interaction contract without an external source or production route state.
...
- `lib/archive/board-clusters.ts` derives primary story islands only from citation-matched relationship support and reviewed story-arc assignments, with deterministic tie order and an explicit uncategorized fallback. `lib/archive/board-layout.ts` keeps small unclustered graphs on the deterministic fallback orbit, while clustered graphs use fixed asymmetric two-dimensional anchors, irregular convex hulls, and an off-center dossier hub. Hub relationships share three evidence trunks before branching from each island's hub-facing boundary; relationship-number controls sit beneath entity markers so they cannot cover names. On wide screens the Board retains bounded zoom/pan, keyboard controls, entity-kind markers, and a direct-connection lens. Below 72rem it falls back to a full-width stacked reading list without trapping touch scrolling.
- Fixture Board quick starts are calculated only from synthetic citation-backed edge counts, with canonical arc order breaking ties. They update the existing arc query and focus the demo map without introducing a production label, entity, or claim.
- The fixture Source Files view exercises the dossier, document-density map, page-frame navigator, paper review surface, and selected-record inspector without a real source identity, source URL, checksum, quotation, or page image. Direct `/sources` navigation begins at the overview; evidence handoffs add `#page-review` and focus the selected demo page behind a three-step reading path. Every interactive frame remains explicitly marked as demo data, and technical fixture identifiers stay in an optional disclosure.
- `lib/archive/source-links.ts` and the shared `SourcePageLink` keep fixture navigation page-exact and review-focused across Receipts, Timeline, Board evidence cards, Trace, and Source Files. They demonstrate the handoff contract without bundling a production source or official-source action.
