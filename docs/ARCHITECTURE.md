...
- `lib/archive/board-clusters.ts` derives primary story islands only from citation-matched relationship support and reviewed story-arc assignments, with deterministic tie order and an explicit uncategorized fallback. `lib/archive/board-layout.ts` keeps small graphs on a degree-centered orbit, groups dense 16–24-node graphs into separated perimeter sectors, and draws a curved ribbon around each island. On wide screens the Board renders the highest-degree node as a circular dossier hub, varies peripheral markers by entity kind, and adds bounded zoom/pan, keyboard controls, and a direct-connection lens. Below 72rem it falls back to a full-width stacked reading list without trapping touch scrolling.
